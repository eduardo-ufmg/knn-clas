...
class KNN(BaseEstimator, ClassifierMixin):
    ...
    def predict(self, X: NDArray[np.float64], k: Optional[int] = None) -> NDArray[np.int64]:
        ...
        nearest = np.argpartition(pairwise_dists, k, axis=1)[:, :k]
    ...
    def likelihood_score(self, X: NDArray[np.float64], k: int) -> Tuple[NDArray[np.float64], NDArray[np.float64]]:
        check_is_fitted(self)
...
class KResults(TypedDict):
    wilcoxon: Dict[str, WilcoxonResult]
    ttest_rel: Dict[str, TTestResult]

DatasetResults = Dict[int, KResults]

StatisticalResults = Dict[str, DatasetResults]
...
def run_statistical_validation(datasets: Dict[str, DatasetType], output_dir: str = 'output') -> None:
    ...
    results: StatisticalResults = {dname: {} for dname in datasets.keys()}
    ...
    for dname, dataset in datasets.items():
        ...
        try:
            ...
            cv_metrics: Dict[int, Dict[str, Dict[str, List[float]]]] = {
                k: {
            ...
            for train_idx, test_idx in KFold(n_splits=10).split(X):
                ...
                for model_name, model in [('KNN', KNN()), ('KNN_CLAS', KNN_CLAS())]:
                    model.fit(X_train, y_train)
                    
                    for k in k_values:
                        y_pred = model.predict(X_test, k)
                        cv_metrics[k][model_name]['accuracy'].append(float(accuracy_score(y_test, y_pred)))
                        cv_metrics[k][model_name]['precision'].append(float(precision_score(y_test, y_pred, zero_division=0)))
                        cv_metrics[k][model_name]['recall'].append(float(recall_score(y_test, y_pred, zero_division=0)))
                        cv_metrics[k][model_name]['f1'].append(float(f1_score(y_test, y_pred, zero_division=0)))
        
            for k in k_values:
                results[dname][k] = {'wilcoxon': {}, 'ttest_rel': {}}
                
                # Wilcoxon test
                wilcoxon_results: Dict[str, WilcoxonResult] = {}
                for metric in ['accuracy', 'precision', 'recall', 'f1']:
                    a = np.array(cv_metrics[k]['KNN'][metric])
                    b = np.array(cv_metrics[k]['KNN_CLAS'][metric])
                    diff = a - b
                    if np.all(diff == 0):
                        wilcoxon_results[metric] = {'statistic': 0.0, 'pvalue': 1.0}
                    else:
                        stat, pvalue = wilcoxon(a, b)
                        wilcoxon_results[metric] = {'statistic': stat, 'pvalue': pvalue}

                results[dname][k]['wilcoxon'] = wilcoxon_results

                # T-test
                ttest_results: Dict[str, TTestResult] = {}
                for metric in ['accuracy', 'precision', 'recall', 'f1']:
                    a = np.array(cv_metrics[k]['KNN'][metric])
                    b = np.array(cv_metrics[k]['KNN_CLAS'][metric])
                    diff = a - b
                ...
                results[dname][k]['ttest_rel'] = ttest_results

        except Exception as e:
            print(f"Error processing {dname}: {e}")
            continue
...
ModelResults = Dict[str, SpatialMetrics]
KResultsSpatial = Dict[int, ModelResults]
SpatialResults = Dict[str, KResultsSpatial]

def run_likelihood_analysis(datasets: Dict[str, DatasetType], output_dir: str = 'output') -> None:
    print("\n=== Start likelihood analysis ===")
    results: SpatialResults = {dname: {} for dname in datasets.keys()}
    ...
    for dname, dataset in datasets.items():
        ...
        try:
            ...
            for k in k_values:
                ...
                for model_name, model in [('KNN', knn), ('KNN_CLAS', knn_clas)]:
                    q0, q1 = model.likelihood_score(X, k)
                    ...
                    if len(points_class0) > 0 and len(points_class1) > 0:
                        distances_opposite = cdist(points_class0, points_class1, metric='euclidean')
                    ...
                    if len(points_class0) > 1:
                        same_distances.extend(pdist(points_class0, metric='euclidean'))
                    if len(points_class1) > 1:
                        same_distances.extend(pdist(points_class1, metric='euclidean'))
                    ...
                    results[dname][k][model_name] = metrics
